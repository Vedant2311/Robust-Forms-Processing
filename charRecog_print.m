...
Ab(:,:,3) = Ab3;
 ...
 % 500:3300,1000:4500 for print_2 
 % 500:3120,: for print_9
 % 1500:3600,1:3456 for print_18
 % 1000:3600,1:3456 for print_44
img = out;
size(img)
img2 = img(1000:3600,1:3456,:);
 img3=img2;
% 1:1400, 1:2000 for print_2
% 1:1300, 1:3000 for print_9
% 1:1200, 1:2750 for print_18
% 1:1200, 1:2500 for print_44
%  img3 = img2(1:1500,1:2500,:);
illuminant = illumpca(img3);
img1(:,:,1) = img3(:,:,1) /illuminant(1);
img1(:,:,2) = img3(:,:,2) / illuminant(2);
img1(:,:,3) = img3(:,:,3) / illuminant(3);
...
%12,100,14,100,1.6,200,no,no,no,no for print_2
%6,100,14,100,2,100,300,500,620,no for print_9
%6,70,15,1200,2,150,430,500,550,1200 for print_18
%6,70,15,150,2.5,100,400,590,670,420 for print_44
  %if thisBB(3)>=6 && thisBB(3)<=70 && thisBB(4)>=15 && thisBB(4)<=150  && thisBB(3)/thisBB(4)<2.5 && thisBB(3)*thisBB(4)>100 && thisBB(2)>400 && (thisBB(2)<590 || thisBB(2)>670)   && thisBB(1)>420
